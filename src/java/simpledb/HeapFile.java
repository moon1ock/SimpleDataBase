...
public class HeapFile implements DbFile {
    ...
    public ArrayList<Page> insertTuple(TransactionId tid, Tuple t)
        ...
        for (int i = 0; i < this.numPages(); i++) {
            HeapPage tuplepage = (HeapPage) Database.getBufferPool().getPage(tid, new HeapPageId(this.getId(), i),
        ...
        // if the flag didn't fall, it means that the page was not added, and we have to
        // create a new one
        if (!flag) {
    ...
    public ArrayList<Page> deleteTuple(TransactionId tid, Tuple t) throws DbException, TransactionAbortedException {
        // some code goes here
        // not necessary for lab1
        ArrayList<Page> deleted = new ArrayList<Page>(); // array to keep track of deleted pages to mark them as diry
                                                         // later
        HeapPage deletedpage = (HeapPage) Database.getBufferPool().getPage(tid, t.getRecordId().getPageId(),
                Permissions.READ_WRITE);
