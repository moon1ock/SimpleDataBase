...
public class Join extends Operator {
    ...
    protected Tuple fetchNext() throws TransactionAbortedException, DbException {
        ...
        if (!flag) {
            ...
            if (children1.size() < children2.size()){ // check which table is smaller in size for efficiency M + (m*N)
            for (Tuple c1 : this.children1) {
                for (Tuple c2 : this.children2) {
                    if (p.filter(c1, c2)) {
                        TupleDesc td = this.getTupleDesc();
                        Tuple tuple = new Tuple(td);
                        int i = 0; // taking the iterating counter out of the loop will serve as the index in the
                                   // array for joined children
            ...
            this.iterator = joinedchildren.iterator();}
                ...
                for (Tuple c1 : this.children2) {
                    for (Tuple c2 : this.children1) {
                        if (p.filter(c1, c2)) {
                            TupleDesc td = this.getTupleDesc();
                            Tuple tuple = new Tuple(td);
                            int i = 0; // taking the iterating counter out of the loop will serve as the index in the
