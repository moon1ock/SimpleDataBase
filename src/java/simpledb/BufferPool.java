...
public class BufferPool {
    ...
    public void deleteTuple(TransactionId tid, Tuple t) throws DbException, IOException, TransactionAbortedException {
        ...
        // delete the tuple
        ArrayList<Page> deletedpages = Database.getCatalog().getDatabaseFile(t.getRecordId().getPageId().getTableId())
                .deleteTuple(tid, t);
    ...
    private synchronized void evictPage() throws DbException {
        ...
        try {
            synchronized (this.pages) {
                ...
                this.evitcounter = (this.evitcounter + 1) % this.pages.size(); // take modulo if counter is bigger than
                                                                               // the buffer pool size
                ArrayList<PageId> pagestoevict = new ArrayList<PageId>(this.pages.keySet()); // creating an array of all
                                                                                             // pages since cannot
                                                                                             // access by index in
                                                                                             // hashmap
